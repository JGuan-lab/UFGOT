...
from utils import solver,solver_ufgot,solver_fgot
# import torch
...
class MegaWass:
    ...
    def solver_megawass_ufgot(
        ...
        log_ent_cost: if log is True, return a list of entropic loss.
        """
        return solver_ufgot(X, Y, px, py, rho, uot_mode, eps, entropic_mode, alpha, D, init_pi, \
                    init_duals, log, verbose, early_stopping_tol, eval_bcd=self.eval_bcd, \
                    eval_uot=self.eval_uot, tol_bcd=self.tol_bcd, nits_bcd=self.nits_bcd, \
                    tol_uot=self.tol_uot, nits_uot=self.nits_uot)

    def solver_megawass_fgot(
        self,
        X,
        Y,
        px=(None, None),
        ...
        log_ent_cost: if log is True, return a list of entropic loss.
        """
        return solver_fgot(X, Y, px, py, rho, uot_mode, eps, entropic_mode, alpha, D, init_pi, \
                    init_duals, log, verbose, early_stopping_tol, eval_bcd=self.eval_bcd, \
                    eval_uot=self.eval_uot, tol_bcd=self.tol_bcd, nits_bcd=self.nits_bcd, \
                    tol_uot=self.tol_uot, nits_uot=self.nits_uot)
    ...
    def solver_ufgot(
        ...
        return self.solver_megawass_ufgot(X, Y, px, py, rho, uot_mode, eps, entropic_mode, alpha, D, \
                                    init_pi, init_duals, log, verbose, early_stopping_tol)

    def solver_fucoot_fgot(
        self,
        X,
        Y,
        px=(None, None),
        ...
        return self.solver_megawass_fgot(X, Y, px, py, rho, uot_mode, eps, entropic_mode, alpha, D, \
                                    init_pi, init_duals, log, verbose, early_stopping_tol)
